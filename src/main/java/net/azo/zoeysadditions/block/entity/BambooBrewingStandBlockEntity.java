...
import net.azo.zoeysadditions.block.custom.BambooBrewingStandBlock;
...
public class BambooBrewingStandBlockEntity extends LockableContainerBlockEntity implements SidedInventory {
    ...
    public BambooBrewingStandBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.BAMBOO_BREWING_STAND, pos, state);
        this.inventory = DefaultedList.ofSize(5, ItemStack.EMPTY);
        this.propertyDelegate = new PropertyDelegate() {
            public int get(int index) {
                switch (index) {
                    case 0:
                        return BambooBrewingStandBlockEntity.this.brewTime;
                    case 1:
                        return BambooBrewingStandBlockEntity.this.fuel;
                    default:
                        return 0;
                }
            }
            ...
            public void set(int index, int value) {
                switch (index) {
                    case 0:
                        BambooBrewingStandBlockEntity.this.brewTime = value;
                        break;
                    case 1:
                        BambooBrewingStandBlockEntity.this.fuel = value;
                }
    ...
    protected Text getContainerName() {
        return Text.translatable("container.zoeysadditions.bamboo_brewing_stand");
    }
    ...
    public static void tick(World world, BlockPos pos, BlockState state, BambooBrewingStandBlockEntity blockEntity) {
        ...
        if (!Arrays.equals(bls, blockEntity.slotsEmptyLastTick)) {
            ...
            if (!(state.getBlock() instanceof BambooBrewingStandBlock)) {
                return;
            }

            for(int i = 0; i < BrewingStandBlock.BOTTLE_PROPERTIES.length; ++i) {
                blockState = (BlockState)blockState.with(BambooBrewingStandBlock.BOTTLE_PROPERTIES[i], bls[i]);
            }
